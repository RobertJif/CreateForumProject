...
{
    public partial class BuatDiskusi_A17TI : System.Web.UI.Page
    {
        string user1;
        ...
        protected void BtnPost_Click(object sender, EventArgs e)
        {
            ...
            SqlCommand cmd = new SqlCommand("Insert into Forum_postg15(username, posttitle, postmessage) values ('" + user1 + "','" + txtposttitle.Text + "', '" + txtpostmessage.InnerText.ToString() + "')", con);
            con.Open();
            cmd.ExecuteNonQuery();
            con.Close();
            Response.Redirect("~/Own_A17TI.aspx");
        ...
        public void knn(string[] kata)
        {
            ...
            for (int i = 0; i < jumlah(); i++)
            {
                ...
                jarak[i] = Math.Pow(kuliah - cari2("kuliah", i + 1), 2) + Math.Pow(ajar - cari2("ajar", i + 1), 2)
                    ...
                    + Math.Pow(barcelona - cari2("barcelona", i + 1), 2)
                    ...
                    + Math.Pow(kpk - cari2("kpk", i + 1), 2) + Math.Pow(minta - cari2("minta", i + 1), 2)
                    + Math.Pow(ingin - cari2("ingin", i + 1), 2)
                    ...
                    + Math.Pow(snapdragon - cari2("snapdragon", i + 1), 2) + Math.Pow(aplikasi - cari2("aplikasi", i + 1), 2)
                    + Math.Pow(cari - cari2("cari", i + 1), 2)
            ...
            else if (a_politik > a_edukasi && a_politik > a_olahraga && a_politik > a_teknologi && a_politik > a_game)
            {
                kat_akhir = "politik";
                SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["FRCon"].ConnectionString);
                SqlCommand cmd = new SqlCommand("Insert into klasifikasi_politik(politik) values ('" + txtposttitle.Text + "')", con);
                SqlCommand cmd2 = new SqlCommand("Insert into semuatopik(topik, kategori) values ('" + txtposttitle.Text + "','" + kat_akhir + "')", con);
                con.Open();
                cmd.ExecuteNonQuery();
                cmd2.ExecuteNonQuery();
            ...
            else if (a_teknologi > a_edukasi && a_teknologi > a_olahraga && a_teknologi > a_politik && a_teknologi > a_game)
            {
                kat_akhir = "teknologi";
                SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["FRCon"].ConnectionString);
                SqlCommand cmd = new SqlCommand("Insert into klasifikasi_teknologi(teknologi) values ('" + txtposttitle.Text + "')", con);
                SqlCommand cmd2 = new SqlCommand("Insert into semuatopik(topik, kategori) values ('" + txtposttitle.Text + "','" + kat_akhir + "')", con);
                con.Open();
                cmd.ExecuteNonQuery();
                cmd2.ExecuteNonQuery();
