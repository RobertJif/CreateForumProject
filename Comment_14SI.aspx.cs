...
{
    public partial class Comment_14SI : System.Web.UI.Page
    {
        string user1;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["FRCon"].ConnectionString);
            SqlDataAdapter da = new SqlDataAdapter("Select * from forum_post15si where postid=" + s, con);
            con.Open();
            ...
            da = new SqlDataAdapter("Select * from komen_si  where postid=" + s, con);
            con.Open();
            ds = new DataSet();
            da.Fill(ds);
            if (ds.Tables[0].Rows.Count > 0)
        ...
        protected void BtnPost_Click(object sender, EventArgs e)
        {
            ...
            String s = Request.QueryString["field1"];
            SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["FRCon"].ConnectionString);
            SqlCommand cmd = new SqlCommand("Insert into komen_si(comment, postid, nama) values ('" + txtComment.Text + "', '" + s + "', '" + user1 + "')", con);
            con.Open();
            cmd.ExecuteNonQuery();
            con.Close();
