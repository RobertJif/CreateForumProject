...
{
    public partial class G15SI_KlasifikasiBeritaKehilangan : System.Web.UI.Page
    {
        string username;
        ...
        {
            SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["FRCon"].ConnectionString);
            SqlDataAdapter da = new SqlDataAdapter("Select * from klasifikasi_berita", con);
            con.Open();
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                {
                    string id = ds.Tables[0].Rows[i]["id_klasifikasi"].ToString();
                    //string author = ds.Tables[0].Rows[i]["username"].ToString();
                    string title = ds.Tables[0].Rows[i]["berita_kehilangan"].ToString();
                    ...
                    btnReadMore.InnerHtml = "<a href=\"Comment_SI.aspx?field1=" + id + "\">Komentar</a>";
                    divreader.Controls.Add(btnReadMore);
